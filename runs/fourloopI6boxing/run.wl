(* =================================================================== *)
(*  Bootstrap Run: fourloopI6boxing                                     *)
(*                                                                     *)
(*  Integrand: -(x[1,2] x[1,4] x[3,7] x[1,3] x[2,4]                    *)
(*              (-x[1,7] x[5,4] + x[1,4] x[5,7] - x[1,5] x[7,4]))      *)
(*              / (13 propagators with 5,6,7,8)                         *)
(*  LS: u*v/(z-zz),  n=0,  poleType="simple"                            *)
...
(* ---- directory layout ---- *)
runDir   = DirectoryName[$InputFileName];           (* runs/fourloopI6boxing/ *)
...
(* ---- global variables ---- *)
$Integrand = -((x[1,2] x[1,4] x[3,7] x[1,3] x[2,4] (-x[1,7] x[5,4] + x[1,4] x[5,7] - x[1,5] x[7,4]))/
  (x[1,5]^2 x[1,7]^2 x[2,6] x[2,7] x[3,5] x[3,6] x[4,5] x[4,7] x[4,8] x[5,8] x[6,7] x[6,8] x[7,8]));

$Perms = {{1,2,3,4},{2,1,3,4},{1,3,2,4},{2,3,1,4},{3,1,2,4},{3,2,1,4}};
label = "fourloopI6boxing";
weightN = -ConformalWeight[$Integrand, 1];
Print["Conformal weight n = ", weightN];

poleType = "simple";
lsAddPole = u * v;  (* base = u*v, primary pole = 1/(z-zz) *)
order = 3;
yOrder = 4;
