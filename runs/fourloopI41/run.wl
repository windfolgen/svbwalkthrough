(* =================================================================== *)
(*  Bootstrap Run: fourloopI41                                           *)
(*                                                                     *)
(*  Integrand: (x[4,7]*x[5,6]-x[4,5]x[6,7]-x[4,6]x[5,7])              *)
(*             / (14 propagators with 5,6,7,8)                          *)
...
ansatzExpr = Import[FileNameJoin[{runDir, "fourloopI41ansatz.m"}]];
...
poleType = "simple";
lsAddPole = 1/(1 - u);  (* base = 1/(1-u), primary pole = 1/(z-zz) *)
order = 4;
yOrder = 5;
...
preflight = ReviewGate[rootDir, label, "preflight"];
If[preflight["Status"] === "FAIL",
  Print["[ABORT] Preflight FAIL: ", Cases[preflight["Checks"], _?(#["Status"] === "FAIL" &)]];
  Return[$Failed]
];
Print[preflight["Status"], ": PASS ", preflight["Summary", "PASS"], ", WARN ", preflight["Summary", "WARN"], ", FAIL ", preflight["Summary", "FAIL"]];

(* ====== Pre-Boundary input checks ====== *)
Print["=== PRE-BOUNDARY CHECKS ==="];
preboundary = ReviewGate[rootDir, label, "preboundary"];
If[preboundary["Status"] === "FAIL",
  Print["[ABORT] Pre-Boundary FAIL: ", Cases[preboundary["Checks"], _?(#["Status"] === "FAIL" &)]];
  Return[$Failed]
];
Print[preboundary["Status"], ": PASS ", preboundary["Summary", "PASS"], ", WARN ", preboundary["Summary", "WARN"], ", FAIL ", preboundary["Summary", "FAIL"]];
...
RunBoundaryConditions[rootDir, label, order, {5,6,7,8}, "InputDir" -> FileNameJoin[{runDir, "boundaries"}]];
...
(* ====== Pre-Series input checks ====== *)
Print[""];
Print["=== PRE-SERIES CHECKS ==="];
preseries = ReviewGate[rootDir, label, "preseries"];
If[preseries["Status"] === "FAIL",
  Print["[ABORT] Pre-Series FAIL: ", Cases[preseries["Checks"], _?(#["Status"] === "FAIL" &)]];
  Return[$Failed]
];
Print[preseries["Status"], ": PASS ", preseries["Summary", "PASS"], ", WARN ", preseries["Summary", "WARN"], ", FAIL ", preseries["Summary", "FAIL"]];
...
(* ====== Pre-Solve input checks ====== *)
Print[""];
Print["=== PRE-SOLVE CHECKS ==="];
presolve = ReviewGate[rootDir, label, "presolve"];
If[presolve["Status"] === "FAIL",
  Print["[ABORT] Pre-Solve FAIL: ", Cases[presolve["Checks"], _?(#["Status"] === "FAIL" &)]];
  Return[$Failed]
];
Print[presolve["Status"], ": PASS ", presolve["Summary", "PASS"], ", WARN ", presolve["Summary", "WARN"], ", FAIL ", presolve["Summary", "FAIL"]];
