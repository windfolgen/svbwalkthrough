results = Get["ls_results.m"];
higherPoles = Get["higher_poles.m"];
normalize[res_] := Module[{normList},
  (* Check for failure conditions *)
  If[res === {},
    Return["WeightDrop"]
  ];
  If[!ListQ[res] || !FreeQ[res, $Failed] || !FreeQ[res, "Failed"] || !FreeQ[res, TimeConstraint] || !FreeQ[res, MemoryLimit],
    Return["Failed"]
  ];
...
normalizedResults = Table[{r[[1]], {normalize[r[[2]]], MemberQ[higherPoles, r[[1]]]}}, {r, results}];

groups = GatherBy[normalizedResults, #[[2]]&];

(* Sort groups by descending number of integrands, but keep "Failed" at the end if possible *)
groups = SortBy[groups, {If[#[[1, 2, 1]] === "Failed", 1, 0], -Length[#]}&];
...
tex = tex <> "\\section{Summary of Results}\nTotal integrands: 412. Total distinct classes: " <> ToString[Length[groups]] <> ".\n\n";

Do[
  grp = groups[[i]];
  val = grp[[1, 2, 1]];
  hasHigherPole = grp[[1, 2, 2]];
  inds = grp[[All, 1]];
  tex = tex <> "\\subsection*{Class " <> ToString[i] <> " (" <> ToString[Length[inds]] <> " integrands)}\n";
  tex = tex <> "Integrand Indices: " <> StringRiffle[ToString /@ inds, ", "] <> "\n\n";
  
  If[hasHigherPole,
     tex = tex <> "\\textbf{Note:} Encountered uncancelled higher poles during evaluation.\\\\[1em]\n\n"
  ];

  If[val === "Failed",
    tex = tex <> "Result: Failed to evaluate / Timeout.\\\\[1em]\n\n",
    If[val === "WeightDrop",
      tex = tex <> "Result: Empty set $\\{\\}$ (Weight drop).\\\\[1em]\n\n",
...
Export["summary_4L.tex", tex, "Text"];
Print["Exported to summary_4L.tex"];
