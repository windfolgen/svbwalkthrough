...
(* Generate tempHashed *)
tp2Replaced = tensorBasis /. rep; (* Wait, rep is the index mapping *)
(* Get rep from record/flag *)
...
temExpr = (result[[k, i + 1]]*(tpRawFromGen[[2]] /. rep) /. {p -> (i + 1)} // Expand) /. {d[a_, b_] :> (d2[1, a] + d2[1, b] - d2[a, b])/2} /. {d2[1, 3] -> 1, d2[1, 2] -> u, d2[2, 3] -> v, d[a_, 1] :> 0} /. {G[i, a_] :> Times @@ (Thread@Power[topArray[[i]], -a])} // Expand;
...
tOpt = SessionTime[];
(* Optimized contraction: tempHashed . Transpose[mat] *)
...
tSlow = SessionTime[];
tpRaw1Replaced = tpRawFromGen[[1]] /. {u -> 1, p -> 3};
