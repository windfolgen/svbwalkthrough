...
(* Redefine top to match ProjectTensor *)
top = {top1, top2};
...
Do[
  ...
  Do[
    ...
    (* Match the exact loops in ProjectTensor *)
    Do[
      ...
      Do[
        ...
        If[dtClass > 0.01 || dtCheck > 0.01,
          Print["      [SLOW] term ", k, " top ", i, " component ", j, " subterm ", l, " Classify: ", dtClass, "s, Check: ", dtCheck, "s"];
        ];
      , {l, 1, Length[tem1List]}];
    , {j, 1, Length[temExpr]}];
  , {i, 1, 2}];
