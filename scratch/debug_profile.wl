...
k = 4034;
i = 2; (* Topology 2 *)
Print["=== Step-by-Step Profiling Term ", k, " Topology ", i, " with G-Hashing ==="];
...
Print["  ClassifyTopology took ", SessionTime[] - tClass, "s"];

tHash = SessionTime[];
glist = Cases[{temp}, _G, Infinity] // DeleteDuplicates;
repG = AssociationMap[h[Hash[#]] &, glist];
tempHashed = temp /. repG;
Print["  G-Hashing took ", SessionTime[] - tHash, "s"];

tVar = SessionTime[];
tensor = Variables[tpRaw[[1]]] // DeleteCases[#, _?((Head[#] =!= vc && Head[#] =!= g) &)] &;
Print["  Variables took ", SessionTime[] - tVar, "s. Variables count: ", Length[tensor]];

tRep = SessionTime[];
tensorActual = tensor /. rep;
Print["  rep on tensor variables took ", SessionTime[] - tRep, "s"];

tRev = SessionTime[];
revRep = Map[Reverse, rep];
tempDummy = tempHashed /. revRep;
Print["  revRep on tempHashed took ", SessionTime[] - tRev, "s"];

tDot = SessionTime[];
resDummy = tempDummy . tpRaw[[1]];
Print["  Dot product took ", SessionTime[] - tDot, "s"];

tRepRes = SessionTime[];
resActual = resDummy /. rep;
Print["  rep on resDummy took ", SessionTime[] - tRepRes, "s"];

tMon = SessionTime[];
temlistOpt = resActual // MonomialList[#, tensorActual] &;
Print["  MonomialList took ", SessionTime[] - tMon, "s"];

Print["=== Finished step-by-step profiling ==="];
