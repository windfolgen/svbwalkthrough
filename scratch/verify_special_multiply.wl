...
(* Save old SpecialMultiply *)
ClearAll[SpecialMultiplyOld];
...
(* Let's test on terms 4030 to 4040 *)
Do[
  Print["\n=== Testing on Term ", k, " ==="];
  ...
  Print["  Old SpecialMultiply took: ", tOld, "s"];
  Print["  New SpecialMultiply took: ", tNew, "s (raw multiplication + factoring)"];
  Print["  New Raw Multiplication took: ", tRaw, "s"];

  diff = resOld - resNew // Expand // Simplify;
  If[diff === 0,
    Print["  SUCCESS: Mathematically identical!"],
    Print["  FAILED: Results differ! Diff: ", Short[diff, 10]]
  ];
, {k, 4030, 4040}];
