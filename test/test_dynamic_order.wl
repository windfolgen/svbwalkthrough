...
(* Series substitution for z1, zz1 to quickly find the u pole *)
...
Print["Calculated u pole for element 3: ", uPole];

yOrderReq = 7 + 2*uPole;
Print["Required Y order: ", yOrderReq];

(* Now compute exactly up to that order using series substitution *)
...
testFinal = term3 /. {z1 -> z1S, zz1 -> zz1S, v -> vS};
testFinal = Normal[Series[testFinal /. {Log[u] -> logU}, {u, 0, 0}, {Y, 0, yOrderReq}]] /. {logU -> Log[u]};
Print["Size of final series string: ", StringLength[ToString[InputForm[testFinal]]]];
