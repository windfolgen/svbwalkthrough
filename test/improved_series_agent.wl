...
result=Reap[Do[
test = temp[[i]] /. {f[5]->Zeta[5],f[7]->Zeta[7],f[3]->Zeta[3],P[0]->-Log[u]};
test = test / (z-zz);
test = Normal[Series[test, {zz, Infinity, 8}, {z, Infinity, 8}]];
test = test /. {zz->1/u/z} // Expand;
test = test /. {Power[z,a_/;(a<0)]:>Power[zz*u,-a]};
test = test /. zrepInf /. {z->(1+u-v-Sqrt[-4 u+(-1-u+v)^2])/(2 u),zz->(1+u-v+Sqrt[-4 u+(-1-u+v)^2])/(2 u)} /. {v->1-Y} // Expand;
test = Expand[test * OptionValue["additional"] * (1/u)];
...
result=Reap[Do[
test = temp[[i]] /. {f[5]->Zeta[5],f[7]->Zeta[7],f[3]->Zeta[3],P[0]->-Log[u/v]};
test = test / (z-zz);
test = Normal[Series[test, {zz, Infinity, 8}, {z, Infinity, 8}]];
test = test /. {zz->v/u/z} // Expand;
test = test /. {Power[z,a_/;(a<0)]:>Power[zz*u/v,-a]};
test = test /. zrepInfP /. {z->(-1+u+v-Sqrt[-4 u v+(-1+u+v)^2])/(2 u),zz->(-1+u+v+Sqrt[-4 u v+(-1+u+v)^2])/(2 u)} /. {v->1-Y} // Expand;
test = Expand[test * OptionValue["additional"] * (1/u)];
...
result=Reap[Do[
test = temp[[i]] /. {f[5]->Zeta[5],f[7]->Zeta[7],f[3]->Zeta[3],I[z,0,0]->Log[u]};
test = test / (z-zz);
test = Normal[Series[test, {z, 0, 8}, {zz, 0, 8}]];
test = test /. {zz->u/z} // Expand;
test = test /. {Power[z,a_/;(a<0)]:>Power[zz/u,-a]};
test = test /. zrep0 /. {z->1/2 (1+u-Sqrt[-4 u+(1+u-v)^2]-v),zz->1/2 (1+u+Sqrt[-4 u+(1+u-v)^2]-v)} /. {v->1-Y} // Expand;
test = Expand[test * OptionValue["additional"] * (1)];
...
result=Reap[Do[
test = temp[[i]] /. {f[5]->Zeta[5],f[7]->Zeta[7],f[3]->Zeta[3],I[z,0,0]->Log[u/v]};
test = test / (z-zz);
test = Normal[Series[test, {z, 0, 8}, {zz, 0, 8}]];
test = test /. {zz->u/z/v} // Expand;
...
test = Expand[test * OptionValue["additional"] * (1/v)];
...
result=Reap[Do[
test = temp[[i]] /. {I[z,1,0]->Log[u/v],f[a_]:>Zeta[a],f[3,3]->Zeta[3]^2/2,f[3,5]->Zeta[3]Zeta[5]-f[5,3]};
test = test /. {-1+z->z1, -1+zz->zz1, z->z1+1, zz->zz1+1};
test = test / (z1-zz1);
test = Normal[Series[test, {z1, 0, 8}, {zz1, 0, 8}]];
test = test /. {zz1->u/v/(z1)} // Expand;
...
result=Reap[Do[
test = temp[[i]] /. {I[z,1,0]->Log[u],f[a_]:>Zeta[a],f[3,3]->Zeta[3]^2/2,f[3,5]->Zeta[3]Zeta[5]-f[5,3]};
test = test /. {-1+z->z1, -1+zz->zz1, z->z1+1, zz->zz1+1};
test = test / (z1-zz1);
test = Normal[Series[test, {z1, 0, 8}, {zz1, 0, 8}]];
test = test /. {zz1->u/(z1)} // Expand;
...
test = test /. {Power[z,a_/;(a<0)]:>Power[zz*u,-a]};
test = test /. zrepInf /. {z->(1+u-v-Sqrt[-4 u+(-1-u+v)^2])/(2 u),zz->(1+u-v+Sqrt[-4 u+(-1-u+v)^2])/(2 u)} /. {v->1-Y} // Expand;
test = Expand[test * OptionValue["additional"] * ((1/u)^2)];
...
test = test /. {Power[z,a_/;(a<0)]:>Power[zz*v/u,-a]};
test = test /. zrep0P /. {z->(-1+u+v-Sqrt[-4 u v+(-1+u+v)^2])/(2 v),zz->(-1+u+v+Sqrt[-4 u v+(-1+u+v)^2])/(2 v)} /. {v->1-Y} // Expand;
test = Expand[test * OptionValue["additional"] * ((1/v)^2)];
