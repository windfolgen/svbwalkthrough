...
(* ZREP definitions *)
zrepInf = Table[{Power[z, i] -> (Power[(1 + u - v - Sqrt[-4 u + (-1 - u + v)^2])/(2 u), i] /. {v -> 1 - Y} // Expand // Collect[#, Power[_, 1/2], Factor] &), Power[zz, i] -> (Power[(1 + u - v + Sqrt[-4 u + (-1 - u + v)^2])/(2 u), i] /. {v -> 1 - Y} // Expand // Collect[#, Power[_, 1/2], Factor] &)}, {i, 1, 10}] // Flatten;
...
(* Test Function for e0 *)
TestE0[expr_] := Module[{res1, res2, test1, test2, diff, t1, t2},
  t1 = First@AbsoluteTiming[
    test1 = (((((expr*1 /. {f[5]->Zeta[5],f[7]->Zeta[7],f[3]->Zeta[3],I[z,0,0]->Log[u]}) /. {zz->u/z} // Expand) /. {Power[z,a_/;(a<0)]:>Power[zz/u,-a]}) / (-Sqrt[-4 u+(1+u-v)^2])) /. zrep0 /. {z->1/2 (1+u-Sqrt[-4 u+(1+u-v)^2]-v),zz->1/2 (1+u+Sqrt[-4 u+(1+u-v)^2]-v)} /. {v->1-Y} // Expand);
...
TestEinf[expr_] := Module[{res1, res2, test1, test2, diff, t1, t2},
  ...
  t2 = First@AbsoluteTiming[
    ...
    test2 = test2 / (z-zz);
    test2 = Normal[Series[test2, {zz, Infinity, 8}, {z, Infinity, 8}]];
...
PickIndices[len_] := Module[{head, mid, tail},
  head = {1, 2, 3};
  mid = Floor[{len/2 - 1, len/2, len/2 + 1}];
  tail = {len - 2, len - 1, len};
  Union[head, mid, tail]
];
